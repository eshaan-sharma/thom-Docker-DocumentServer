...
RUN    wget -q -P /tmp "https://github.com/eshaan-sharma/unlimited-onlyoffice-package-builder/releases/download/onlyoffice-unlimited-build-debian-11/${OOU_VERSION_MAJOR}.${OOU_BUILD}/onlyoffice-documentserver_${OOU_VERSION_MAJOR}-${OOU_BUILD}-esdnc_amd64.deb" && \
    apt-get -y update && \
    service postgresql start && \
    apt-get -yq install /tmp/onlyoffice-documentserver_${OOU_VERSION_MAJOR}-${OOU_BUILD}-esdnc_amd64.deb && \
    service postgresql stop
...
RUN   chmod 755 /etc/init.d/supervisor && \
    ...
    service supervisor stop && \
    chmod 755 /app/ds/*.sh && \
    rm -f /tmp/onlyoffice-documentserver_${OOU_VERSION_MAJOR}-${OOU_BUILD}-esdnc_amd64.deb && \
